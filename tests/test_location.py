...
with patch('django.contrib.gis.geoip2.GeoIP2'):
    from saas_auth.location.geoip2 import GeoIP2Backend

from django.test import RequestFactory, SimpleTestCase

from saas_auth.location.base import BaseBackend
from saas_auth.location.cloudflare import CloudflareBackend
...
class TestLocationBackends(SimpleTestCase):
    ...
    @patch('saas_auth.location.geoip2._geo')
    def test_geoip2_backend(self, mock_geo):
        ...
        # Test IP is None
        with patch('saas_auth.location.geoip2.get_client_ip', return_value=None):
            data = backend.resolve_location(request)
